...
public class HerobrineWanderRenderer extends MobRenderer<HerobrineWanderingEntity> {

	public HerobrineWanderRenderer(HerobrineWanderModel model, float shadow) {
		super(model, shadow);
	...
	@Override
	public void render(Tessellator tessellator, HerobrineWanderingEntity entity, double x, double y, double z, float yaw, float partialTick) {
		GL11.glEnable(3042);
	...
	private boolean setEyeBrightness(HerobrineWanderingEntity herobrineEntity, int i, float f) {
		if (i == 0) {
	...
	@Override
	protected boolean prepareArmor(HerobrineWanderingEntity entity, int layer, float partialTick) {
		return setEyeBrightness(entity, layer, partialTick);
	}

	@Override
	protected void setupScale(HerobrineWanderingEntity entity, float partialTick) {
		float scale = 0.9375F;
		GL11.glScalef(scale, scale, scale);
	}

	@Override
	protected float getMaxDeathRotation(HerobrineWanderingEntity entity) {
		return 90.0F;
	}
